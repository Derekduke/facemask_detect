...
int main(int argc, const char **argv) {
  ...
  for (auto tensor : inputs) {
    ...
    GetImage(tensor.DataSize() , input_data , gImage_img_test1);
  }
...
int* single_class_non_max_suppression(float* bboxes, float* confidences)
{
  ...
  for(int i=0 ; i<5972 ; i++)
  {
    ...
    if(confidences[i] > conf_thresh)
    {
      ...
      conf_id_sort[cnt].box = temp_box;
      //printf("confid=%d, xmin=%f, ymin=%f, xmax=%f, ymax=%f\n",conf_keep_idx, conf_id_sort[cnt].box.xmin, conf_id_sort[cnt].box.ymin,conf_id_sort[cnt].box.xmax,conf_id_sort[cnt].box.ymax); 
  ...
  for(int i=0 ; i<cnt ; i++)
  {
      printf("%d 's conf is = %f\n" , (conf_id_sort[i]).id , (conf_id_sort[i]).conf);
  }
  printf("\n");
  int pick[5972]={0};
  pick_id=0;
  ...
  while(cnt_remain>0)
  {
    ...
    {
      float overlap_xmin_temp = max_value(conf_id_sort[k].box.xmin , conf_id_sort[r].box.xmin);
      float overlap_ymin_temp = max_value(conf_id_sort[k].box.ymin , conf_id_sort[r].box.ymin);
      float overlap_xmax_temp = min_value(conf_id_sort[k].box.xmax , conf_id_sort[r].box.xmax);
      float overlap_ymax_temp = min_value(conf_id_sort[k].box.ymax , conf_id_sort[r].box.ymax);
